...
from modules.adopter import Adapter_Layer, Adapter
...
class FocalModulation(nn.Module):
    def __init__(self, dim, focal_window, focal_level,
                 focal_factor=2, bias=True, proj_drop=0.,
...
class FocalNetBlock(nn.Module):
    ...
    def __init__(self, dim, input_resolution, mlp_ratio=4., drop=0., drop_path=0.,
                 ...
                 fine_tune_modulation_block_adopter=False
                 ):
        super().__init__()
        self.dim = dim
        ...
        if fine_tune_modulation_block_adopter:
    ...
    def __init__(self, dim, out_dim, input_resolution, depth,
        ...
        self.blocks = nn.ModuleList([
            FocalNetBlock(
                ...
                fine_tune_modulation_block_adopter=fine_tune_modulation_block_adopter
            )
...
class FocalNet(nn.Module):
    ...
    # def __init__(self,**kwargs):
        ...
        self.use_adopter = fine_tune_modulation_block_adopter
        self.freezer()
    def freezer(self):
        if self.use_adopter:
            for name, param in self.named_parameters():
                if "adopter_layer" in name:
    ...
    def forward_features(self, x):
        ...
        for idx_layer, layer in enumerate(self.layers):
            ...
            if idx_layer < self.num_layers - 1:
                pass
                # outputs[idx_layer] = x.detach()
                # outputs[idx_layer] = x.clone()
                # outputs[idx_layer] = x
        x = self.norm(x)  # B L C
        # outputs[self.num_layers - 1] = x.detach()
        # outputs[self.num_layers - 1] = x.clone()
        # outputs[self.num_layers - 1] = x

        # these two must me uncommented
